...
class RandomFlip(object):
    def __init__(self, p=0.5, elem_name='image'):
        ...
        self.transform = A.Compose([
            ...
            A.Rotate([-5, 5], p=self.p),
        ],  keypoint_params=A.KeypointParams(format='xy', remove_invisible=False))

    def __call__(self, sample):
        transformed = self.transform(
            image=np.array(sample['image']), 
            keypoints=np.array(sample["landmarks"]).reshape(-1, 2)
        )
        sample['image'], sample["landmarks"] = transformed['image'], torch.Tensor(transformed['keypoints'])

        return sample

class RandomImgTransforms(object):
    def __init__(self, p=0.25, elem_name='image'):
        self.p = p
        self.elem_name = elem_name
        self.transform = A.Compose([
            # A.GaussNoise(p=self.p, var_limit=(10.0, 30.0)),
            A.RandomBrightnessContrast(p=self.p / 2),
            # Чёрные прямоугольники - имитация посторонних предметов.
            # A.CoarseDropout(p=self.p, min_holes=4, max_holes=10, min_width=20, max_width=50, min_height=20, max_height=50), 
            A.ToGray(p=self.p / 2)
        ])

    def __call__(self, sample):
        transformed = self.transform(image=np.array(sample['image']))
