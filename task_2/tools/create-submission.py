...
def main(args):
    ...
    image_w, image_h = list(map(int, DEFAULT_INPUT_SIZE.split('x')))
    ...
    recognition_model = get_recognition_model(image_w, image_h)
    ...
    for i, file_name in enumerate(tqdm.tqdm(files)):
        ...
        for box in boxes:
            ...
            tensor = prepare_for_recognition(crop, (image_w, image_h)).to(device)
            with torch.no_grad():
